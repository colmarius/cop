...
copContextRegistry = {
  ...
  ,publish: function(context, publicName)
  {
    ...
    {
      //console.log("Publishing context:", context, "under public name:", publicName);
...
/**
 ...
 deactivation, and adaptation requests (adapt an object with a number of traits 
 under a certain context).
 He dictates the trait composition for all adapted objects. It also manages 
 conflicts by applying the right policy (default one for now).
 ...
 Stores this policy as the most recent one. The previous policy will be called 
 if this new policy fails.
 
 - `register(context)`
 
 Automatically when a context gets created it is implicitly registered to the
 context manager. If it is a simple context and it has a name property not 
 empty, then the context manager tries to publish the context.
    ...
    if( this.defaultContext.adaptedObjects.containsKey(object) )
    {
      console.log( "Default context already has an adaptation for object:", 
    ...
    else  
        {
          ...
          var objectFlatten = {};
          for( var name in object ) { objectFlatten[name] = object[name]; }
    ...
    context.adaptedObjects.each( function( object ) {
      ...
      if( traits.length >= 1 ) 
      {
        ...
        if( copTrait.hasConflicts( composedTrait ) ) 
        {
          ...
          policy = (context instanceof copCombinedContext && context.policy 
...
copContext.prototype = {
  ...
  ,adapt: function(var_args)
  {
    ...
    if (this.adaptedObjects.containsKey(object))
    {
      ...
      console.log( "Skipping adaptation for object:", object, "Already has one under context:", this.toString() );
      return;
    }
